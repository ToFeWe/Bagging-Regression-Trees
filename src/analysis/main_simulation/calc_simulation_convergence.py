...
def simulate_convergence(general_settings, convergence_settings, model):
    ...
    output_convergence = np.ones((n_bootstraps_array.shape[0], SIZE_MSE_DECOMP)) * np.nan
    ...
    for index, n_bootstrap in enumerate(n_bootstraps_array):
        output_convergence[index,:] = simulation_basis.calc_mse(ratio=general_settings['BAGGING_RATIO'],
                                                   bootstrap=True,
                                                   min_split_tree=general_settings["min_split_tree"],
                                                   B_iterations = n_bootstrap)

    # Simulate MSE for a high number of bootstrap iterations to visualize its convergence
    output_large_B = simulation_basis.calc_mse(ratio=general_settings['BAGGING_RATIO'],
                                               bootstrap=True,
                                               min_split_tree=general_settings["min_split_tree"],
...
if __name__ == '__main__':
    ...
    output_simulation = simulate_convergence(general_settings_imported, convergence_settings_imported, dgp_model)
